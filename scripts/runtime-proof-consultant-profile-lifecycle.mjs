...
try {
  ...
  const consultantToken = jwt.sign(
    {
  ...
  const checks = [];
  checks.push({
    name: "consultant_get_anon_unauthorized",
    expect: 401,
    ...(await req("/api/consultants/profile")),
  });
  checks.push({
    name: "consultant_patch_user_ok",
    expect: 200,
    ...(await req("/api/consultants/profile", {
      method: "PATCH",
  ...
  checks.push({
    name: "consultant_get_user_ok",
    expect: 200,
    ...(await req("/api/consultants/profile", { cookie: consultantCookie })),
  });
  checks.push({
    name: "consultant_patch_employer_forbidden",
    expect: 403,
    ...(await req("/api/consultants/profile", {
      method: "PATCH",
  ...
  try {
    ...
    if (consultantId) {
      await db
        ...
        .deleteMany({ userId: String(consultantId) });
