...
try {
  ...
  await users.insertOne({
    ...
    email,
    accountType: "user",
    fullName: "Proof WB User",
    createdAt: now,
    updatedAt: now,
  });

  const token = jwt.sign(
    { userId: String(userId), email, accountType: "user" },
    secret,
    { expiresIn: "1h" },
  );
  ...
  const checks = [];
  checks.push({
    name: "transactions_anon_unauthorized",
    expect: 401,
    ...(await req("/api/wealth-builder/transactions")),
  });
  ...
  checks.push({
    name: "transactions_list_auth",
    expect: 200,
    ...(await req("/api/wealth-builder/transactions?limit=5", { cookie })),
  });

  if (txId) {
    checks.push({
      name: "transactions_patch_auth",
      expect: 200,
      ...(await req(`/api/wealth-builder/transactions/${txId}`, {
        method: "PATCH",
        cookie,
        body: { amount: 55.25, notes: "proof lifecycle updated" },
      })),
    });
    checks.push({
      name: "transactions_delete_auth",
      expect: 200,
      ...(await req(`/api/wealth-builder/transactions/${txId}`, {
        method: "DELETE",
        cookie,
