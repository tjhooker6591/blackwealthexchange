...
try {
  ...
  const imageProof = await page.evaluate(() => {
    ...
    const productLinks = Array.from(document.querySelectorAll('a[href^="/marketplace/product/"]'));
    const loadedImgs = imgs.filter(i => i.complete && (i.naturalWidth || 0) > 0);
    return {
      totalImgs: imgs.length,
      loadedImgs: loadedImgs.length,
      productCardLinks: productLinks.length,
      sample: imgs.slice(0,6).map(i => ({ src: i.getAttribute('src'), w: i.naturalWidth, h: i.naturalHeight }))
    };
  });
  ...
  await page.goto(`${base}/financial-literacy`, { waitUntil: 'domcontentloaded' });
  ...
  await page.goto(`${base}/marketplace/become-a-seller`, { waitUntil: 'domcontentloaded' });
  ...
  await page.fill('input[placeholder="Website (optional)"]', 'https://example.com');
  await page.fill('textarea[placeholder="Tell buyers about your business"]', 'Flow proof seller onboarding test');
  ...
  out.steps.push({
    flow: 'become-seller',
    finalUrl: sellerUrl,
    reachedStripe: sellerUrl.includes('connect.stripe.com') || sellerUrl.includes('stripe.com'),
    continuationVisible:
