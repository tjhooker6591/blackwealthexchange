...
async function proofDSignupRedirectGuard(browser) {
  ...
  await page.waitForSelector("text=Unexpected onboarding destination. Please retry.");
  const stayedOnSignup = /\/signup/.test(new URL(page.url()).pathname);

  await page.screenshot({ path: path.join(outDir, "d-signup-redirect-guard.png"), fullPage: true });
  await context.close();
...
async function proofDAdminUnauthorizedMessage(browser) {
  ...
  await page.waitForSelector("text=Admin access required to view user records.");
  await page.screenshot({ path: path.join(outDir, "d-admin-unauthorized-message.png"), fullPage: true });
...
async function proofFNextBestAction(browser) {
  ...
  await page.click('a[href="/wealth-builder/transactions"]:has-text("Do this now")');
  await page.waitForURL("**/wealth-builder/transactions");

  await page.screenshot({ path: path.join(outDir, "f-next-best-action.png"), fullPage: true });
