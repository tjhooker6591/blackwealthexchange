...
import { BLACK_CARD_TIER_BY_ITEM_ID, type BlackCardTier } from "@/lib/black-card";
...
async function allocateCardSequence(db: Db, tier: BlackCardTier, issuedAt: Date) {
  ...
  const checksum = Buffer.from(canonical).toString("base64url").slice(0, 2).toUpperCase();
...
export async function ensureBlackCardMembershipAndCard(params: {
  ...
  const { db, userId, email, stripeSessionId, paymentIntentId, itemId, paidAt, planExpiresAt } = params;
  ...
  if (!membership?._id) return { ok: false, reason: "MEMBERSHIP_UPSERT_FAILED" as const };

  const cardCollection = db.collection("black_card_cards");
  let card = await cardCollection.findOne({ membershipId: String(membership._id), issueVersion: 1 });
  if (!card) {
    ...
    card = await cardCollection.findOne({ membershipId: String(membership._id), issueVersion: 1 });
  }
  ...
  if (userId && ObjectId.isValid(userId)) {
    await db.collection("users").updateOne({ _id: new ObjectId(userId) }, { $set: mirrorPatch });
  }
  if (email) {
