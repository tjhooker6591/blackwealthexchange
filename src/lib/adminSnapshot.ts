...
export async function buildLiveAdminMetrics(db: Db) {
  ...
  const growth = {
    newUsersThisMonth: await safeCount(db, "users", { createdAt: { $gte: month } }),
    newBusinessesThisMonth: await safeCount(db, "businesses", { createdAt: { $gte: month } }),
  };
  const trustSafety = {
    pendingBusinessApprovals: await safeCount(db, "businesses", { status: { $in: ["pending", "pending_review"] } }),
    disputes: await safeCount(db, "disputes", {}),
  };
  const systemHealthLogs = await db.collection("system_health_logs").find({}).sort({ createdAt: -1 }).limit(500).toArray().catch(() => []);
  const failures = systemHealthLogs.filter((x: any) => x.status === "fail" || x.httpStatus >= 500);
  const systemHealth = {
  ...
  const executiveSignals = {
    goingWell: Number(revenue.revenueThisMonth.value || 0) > 0 ? ["Revenue streams active"] : [],
    needsAttention: Number(support.escalated.value || 0) > 0 ? [`${support.escalated.value} escalated tickets`] : [],
    atRisk: failures.length > 0 ? ["System failures detected"] : [],
  };

  return { now, revenue, support, growth, trustSafety, systemHealth, executiveSignals };
}
