...
} from "../../../../.tmp/founding-membership-testable.mjs";
...
async function testClaimableBusinessValidation() {
  const docs = [
    { _id: "approved-1", business_name: "Approved Business", status: "approved", directoryVisibilityApproved: true },
    { _id: "verified-1", business_name: "Verified Business", status: "verified", directoryVisibilityApproved: true, verified: true },
    { _id: "claim-1", business_name: "Claim Started", status: "approved", directoryVisibilityApproved: true, claimStage: "claim_initiated" },
    { _id: "review-1", business_name: "Review Pending", status: "approved", directoryVisibilityApproved: true, claimStage: "ownership_review_pending" },
    { _id: "member-1", business_name: "Already Member", status: "approved", directoryVisibilityApproved: true, claimStage: "founding_growth_member" },
  ];
...
function testAvailabilityNormalization() {
  const available = getFoundingMembershipAvailability({ status: "approved", claimStage: null });
  assert.equal(available.claimable, true);
  assert.equal(available.unavailableReason, null);

  const member = getFoundingMembershipAvailability({ status: "approved", claimStage: "founding_growth_member" });
