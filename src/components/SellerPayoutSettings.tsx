...
export function SellerPayoutSettings({ sellerId }: { sellerId: string }) {
  ...
  return (
    <div className="space-y-4">
      <p>
        <strong>Payouts enabled:</strong> {account.payouts_enabled ? 'Yes' : 'No'}<br/>
      ...
      {account.payouts_enabled && (
        <form
          onSubmit={async e => {
            ...
            // re-fetch
            const refreshed = await fetch(`/api/stripe/account-status?sellerId=${sellerId}`)
              .then(r => r.json());
        ...
        >
          <label>
            ...
            <select name="interval" defaultValue={account.settings.payouts.schedule.interval}>
