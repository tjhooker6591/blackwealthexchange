    ...
    <main className="min-h-screen bg-black p-6 text-white">
      <div className="mx-auto max-w-6xl">
        <div className="mb-5 flex items-center justify-between">
          <h1 className="text-3xl font-black text-[#D4AF37]">Affiliate Attribution</h1>
        ...
        {!loading && !error ? (
          <div className="grid gap-6 lg:grid-cols-2">
            <section className="rounded-xl border border-white/10 bg-white/5 p-4">
              <h2 className="text-lg font-bold text-[#D4AF37]">Recent Clicks</h2>
              <div className="mt-3 overflow-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {clicks.length === 0 ? (
                      <tr><td className="p-2 text-white/60" colSpan={3}>No click data yet.</td></tr>
                    ) : clicks.map((r) => (
                      <tr key={r._id} className="border-t border-white/10">
                        <td className="p-2">{r.affiliateName || r.affiliateId}</td>
                        <td className="p-2">{r.affiliateEmail || "-"}</td>
                        <td className="p-2">{r.clickedAt ? new Date(r.clickedAt).toLocaleString() : "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <section className="rounded-xl border border-white/10 bg-white/5 p-4">
              <h2 className="text-lg font-bold text-[#D4AF37]">Recent Conversions</h2>
              <div className="mt-3 overflow-auto">
                <table className="w-full text-sm">
                  <thead className="text-left text-white/60">
                    <tr>
                  ...
                  <tbody>
                    {conversions.length === 0 ? (
                      <tr><td className="p-2 text-white/60" colSpan={4}>No conversion data yet.</td></tr>
                    ) : conversions.map((r) => (
                      <tr key={r._id} className="border-t border-white/10">
                        <td className="p-2">{r.affiliateName || r.affiliateId}</td>
                        <td className="p-2">${Number(r.amount || 0).toFixed(2)}</td>
                        <td className="p-2">${Number(r.commission || 0).toFixed(2)}</td>
                        <td className="p-2">{r.convertedAt ? new Date(r.convertedAt).toLocaleString() : "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
