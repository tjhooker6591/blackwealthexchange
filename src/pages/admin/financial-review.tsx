      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Section title="Revenue Streams">
          {[
            ...
            [
              "Consulting / Opportunity Network Revenue",
        ...
        <Section title="Transaction Review">
          <pre className="text-xs overflow-auto">
            {JSON.stringify(data?.latestTransactions || [], null, 2)}
          </pre>
        </Section>
        <Section title="Pending Payments / Failed Payments">
          <pre className="text-xs overflow-auto">
            {JSON.stringify(
              {
                pendingRevenue: data?.pendingRevenue,
                failedOrRefunded: data?.failedOrRefunded,
              },
              null,
              2,
            )}
        ...
        <Section title="Refunds / Disputes">
          <p className="text-sm text-zinc-300">
            Using payment status fields only. If no dispute feed is connected,
            values remain zero.
          </p>
        </Section>
        <Section title="Monthly Revenue Trend">
          <p className="text-sm text-zinc-300">
            Use existing Platform Analytics monthly series. This page is central
            finance control for stream-level review.
          </p>
        </Section>
        <Section title="Export / Report Preparation">
          <p className="text-sm text-zinc-300">
            Latest transactions are shown from real payment records only, no
            synthetic values.
          </p>
        </Section>
      </div>
...
function Card({ t, v }: { t: string; v: any }) {
  return (
    <div className="rounded border border-zinc-800 bg-zinc-950 p-4">
      ...
      <p className="text-xl text-yellow-300 font-bold">{money(v)}</p>
    </div>
  );
}
