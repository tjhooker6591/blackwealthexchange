...
export default function LeavingPage() {
  ...
  const to = useMemo(() => safeTarget(router.query.to as any), [router.query.to]);
  ...
  return (
    <main className="min-h-screen bg-black px-4 py-10 text-white">
      ...
      <div className="mx-auto max-w-xl rounded-2xl border border-white/10 bg-white/5 p-6">
        <h1 className="text-2xl font-bold text-[#D4AF37]">You’re leaving BWE</h1>
        ...
        <div className="mt-5">
          <Link href="/business-directory" className="text-sm text-white/80 underline">
            ← Back to directory
          </Link>
        </div>
      </div>
