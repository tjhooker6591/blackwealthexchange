...
export default function CategoryDirectoryLanding() {
  ...
  const raw = typeof router.query.category === "string" ? router.query.category.toLowerCase() : "";
  const category = raw.slice(0, 60);
  const categoryLabel = labelFromSlug(category) || "Black-owned businesses";

  const canonical = canonicalUrl(`/black-owned-businesses/category/${category}`);
    ...
    <>
      ...
      <main className="min-h-screen bg-black px-6 py-12 text-white">
        <div className="mx-auto max-w-4xl">
          <p className="text-xs uppercase tracking-[0.2em] text-yellow-300">Category landing</p>
          <h1 className="mt-3 text-4xl font-extrabold">{categoryLabel} in the Black business directory</h1>
          <p className="mt-4 text-zinc-300">
            Start with this category landing and jump into filtered directory and Travel Map search.
          </p>
