...
export default async function handler(
  ...
  try {
    ...
    const hostHeader = String(
      ...
      .trim();
    const proto =
      protoHeader || (hostHeader.includes("localhost") ? "http" : "https");
    const base = hostHeader
      ? `${proto}://${hostHeader}`
      : "https://www.blackwealthexchange.com";
    ...
    if (error?.code === 11000) {
      return res.status(409).json({
        ok: false,
        error: "That email is already registered for the challenge.",
      });
    }
    console.error("[/api/challenge/join]", error);
    return res.status(500).json({
      ok: false,
      error: "Unable to join challenge right now. Please try again.",
    });
  }
}
