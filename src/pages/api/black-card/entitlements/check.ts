...
import { getBlackCardSession, isTierAllowed, type BlackCardTier } from "@/lib/black-card-member";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const active = String(user?.blackCardStatus || "inactive").toLowerCase() === "active";
  const tier = typeof user?.blackCardTier === "string" ? user.blackCardTier.toLowerCase() : null;
  const allowed = active && isTierAllowed(tier, minimumTier);

  return res.status(200).json({ ok: true, active, tier, benefit, minimumTier, allowed });
}
