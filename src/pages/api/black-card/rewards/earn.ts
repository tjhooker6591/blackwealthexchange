...
import { BLACK_CARD_EARN_RULES, getBlackCardSession } from "@/lib/black-card-member";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!actionType || !referenceId) {
    return res.status(400).json({ ok: false, error: "actionType and referenceId are required" });
  }

  const points = BLACK_CARD_EARN_RULES[actionType];
  if (!points) {
    return res.status(400).json({ ok: false, error: "Unsupported rewards action" });
  }
  ...
  const existing = await db.collection("black_card_reward_actions").findOne({ actionKey });
  if (existing) {
    return res.status(409).json({ ok: false, error: "Duplicate rewards action", code: "DUPLICATE_ACTION" });
  }

  const user = await db.collection("users").findOne(
    ObjectId.isValid(session.userId) ? { _id: new ObjectId(session.userId) } : { email: session.email },
    { projection: { blackCardTier: 1, blackCardStatus: 1, blackCardRewardsBalance: 1 } },
  );
