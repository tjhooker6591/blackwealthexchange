...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { sellerId, amount, paymentMethod, paymentDetails } = JSON.parse(req.body);
    ...
    return res.status(200).json({ message: 'Payout request submitted successfully.', payoutId: result.insertedId });
