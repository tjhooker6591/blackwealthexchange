...
export default async function handler(
  ...
  if (req.method === "PATCH") {
    ...
    if (typeof body.goalName === "string")
      update.goalName = body.goalName.trim();
    if (body.targetAmount !== undefined)
      update.targetAmount = toNonNegativeNumber(body.targetAmount, 0);
    if (body.currentAmount !== undefined)
      update.currentAmount = toNonNegativeNumber(body.currentAmount, 0);
    if (body.targetDate !== undefined)
      update.targetDate = toDateOrNull(body.targetDate);
    if (body.monthlyContributionTarget !== undefined) {
      update.monthlyContributionTarget = toNonNegativeNumber(
  ...
  if (req.method === "DELETE") {
    ...
    if (!existing) {
      return res
        .status(404)
        .json({ ok: false, message: "Goal record not found." });
    }
