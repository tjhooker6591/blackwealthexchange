...
export default async function handler(
  ...
  if (!id) {
    return res
      .status(400)
      .json({ ok: false, message: "Transaction id is required." });
  }
  ...
  if (!objectId) {
    return res
      .status(400)
      .json({ ok: false, message: "Invalid transaction id." });
  }
  ...
  if (req.method === "PATCH") {
    ...
    if (body.amount !== undefined)
      update.amount = toNonNegativeNumber(body.amount, 0);
    if (typeof body.category === "string")
      update.category = body.category.trim();
    if (typeof body.merchant === "string")
      update.merchant = body.merchant.trim();
  ...
  if (req.method === "DELETE") {
    ...
    if (!existing) {
      return res
        .status(404)
        .json({ ok: false, message: "Transaction record not found." });
    }
