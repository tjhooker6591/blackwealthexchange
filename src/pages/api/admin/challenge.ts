...
export default async function handler(
  ...
  try {
    ...
    if (String(req.query.format || "").toLowerCase() === "csv") {
      ...
      const csv = [
        header.map(esc).join(","),
        ...rows.map((r: string[]) => r.map(esc).join(",")),
      ].join("\n");
      res.setHeader("Content-Type", "text/csv; charset=utf-8");
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=challenge-members.csv",
      );
    ...
    return res
      .status(500)
      .json({ ok: false, error: "Unable to load challenge dashboard data." });
  }
}
