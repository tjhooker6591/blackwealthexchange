...
const ALLOWED = ["new", "reviewed", "shortlisted", "contacted", "rejected"] as const;
type HiringStatus = (typeof ALLOWED)[number];

function asStatus(v: unknown): HiringStatus | null {
  if (typeof v !== "string") return null;
  const x = v.trim().toLowerCase();
  return (ALLOWED as readonly string[]).includes(x) ? (x as HiringStatus) : null;
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let payload: { email: string; accountType?: string };
  try {
  ...
  const applicantId = typeof req.body?.applicantId === "string" ? req.body.applicantId : "";
  ...
  try {
    ...
    const applicant = await db
      ...
      .findOne({ _id: new ObjectId(applicantId) }, { projection: { jobId: 1 } });
    ...
    return res.status(200).json({ success: true, hiringStatus: nextStatus });
  } catch (error) {
