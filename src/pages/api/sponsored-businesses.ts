...
const HOUSE_SPONSOR_IMAGE_MAP: Record<string, string> = {
  ...
  "Thomas Hooker Publisher": "/images/sponsors/thomas hooker publisher.jpg",
...
const SPONSOR_IMAGE_ALIASES: Record<string, string> = {
  "/images/sponsors/guardiansoftheforgottenrealm.jpg":
    "/images/sponsors/Guardiansoftheforgottenrealm.jpg",
...
function normalizeSponsorImagePath(raw: string) {
  const value = s(raw);
  if (!value) return HOUSE_SPONSOR_FALLBACK;
  return SPONSOR_IMAGE_ALIASES[value] || value;
}
...
export default async function handler(
  ...
  try {
    ...
    const scheduled = await db
      ...
      .sort({ status: 1, queueStatus: 1, createdAt: 1 })
      .limit(12)
      .toArray();
