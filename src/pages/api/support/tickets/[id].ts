...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
 ...
 const id=String(req.query.id||"").trim(); if(!id) return err(res,400,"MISSING_ID","ticket id required");
 const db=(await clientPromise).db(getMongoDbName());
 const ticket=await db.collection("support_tickets").findOne({ticketId:id,email:u.email},{projection:{internalNotes:0}});
