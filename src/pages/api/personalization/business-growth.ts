...
import { resolveBusinessGrowthCommandCenter } from "@/lib/personalization/businessGrowth";
...
export default async function handler(
  ...
  try {
    ...
    const result = await resolveBusinessGrowthCommandCenter(db, { businessId });
    return res.status(result.ok ? 200 : 404).json(result);
  } catch (err) {
    console.error("[api/personalization/business-growth] error:", err);
