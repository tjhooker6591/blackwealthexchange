...
export default function RealEstateToolkit() {
  ...
  const rto = useMemo(() => { const m = num(rtoYears)*12; const credits = num(rtoCredit)*m+num(rtoOptionFee); return { totalRent: num(rtoRent)*m, totalCredits: credits, remainingGap: Math.max(num(rtoTargetPrice)-credits,0) }; }, [rtoRent, rtoCredit, rtoOptionFee, rtoTargetPrice, rtoYears]);
