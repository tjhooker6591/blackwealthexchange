...
export default function BlackCardJoinPage() {
  ...
  const tier = useMemo(
    () => normalizeTier(typeof router.query.tier === "string" ? router.query.tier : "standard"),
    [router.query.tier],
  );
  ...
  async function startCheckout() {
    if (!user) {
      router.push(`/login?next=${encodeURIComponent(`/black-card/join?tier=${tier}`)}`);
      return;
    }
    ...
    try {
      const res = await fetch("/api/stripe/checkout", {
        ...
        credentials: "include",
        body: JSON.stringify({ type: "plan", itemId: tierConfig.checkoutItemId }),
      });
    ...
    <>
      ...
      <main className="min-h-screen bg-black text-white px-4 py-10">
        <div className="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-white/5 p-6">
          <h1 className="text-3xl font-extrabold text-yellow-200">Join {tierConfig.label}</h1>
          <p className="mt-2 text-white/80">{tierConfig.tagline}</p>
          <p className="mt-4 text-4xl font-black">{tierConfig.monthlyLabel}<span className="text-base font-medium text-white/70">/month</span></p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link href="/black-card" className="rounded-lg border border-white/20 px-4 py-2">Back to Black Card</Link>
          </div>
