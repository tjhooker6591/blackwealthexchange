...
const TOPICS = [
  {
    ...
    anchor: "search Black-owned businesses",
  },
  {
      ...
      <main className="min-h-screen bg-black px-6 py-12 text-white">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mt-4 max-w-3xl text-base text-zinc-300">
            Black wealth grows when circulation, ownership, earnings, and systems work together. This guide
            focuses on action: find Black-owned businesses, increase income pathways, and run a personal
            money system that compounds over time.
          </p>

          <section className="mt-8 grid gap-4 md:grid-cols-3">
            {TOPICS.map((topic) => (
              <article key={topic.title} className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
                <h2 className="text-lg font-semibold text-yellow-200">{topic.title}</h2>
                <p className="mt-2 text-sm text-zinc-300">{topic.description}</p>
                <Link
                  href={topic.href}
          ...
          <section className="mt-10 rounded-2xl border border-white/10 bg-zinc-950 p-5">
            <h2 className="text-2xl font-bold">How Black Wealth Exchange helps</h2>
            <ul className="mt-4 list-disc space-y-2 pl-6 text-zinc-300">
              <li>Directory + Travel Map for trusted business discovery and local economic circulation.</li>
              <li>Wealth Builder for monthly cash-flow clarity, debt reduction, and savings momentum.</li>
              <li>Consultant and employer workflows to improve access to real opportunities.</li>
              <li>Search-first platform design so users can move from intent to action quickly.</li>
            </ul>
          </section>
        </div>
      </main>
