...
export default function ChallengePage() {
  ...
  return <main className="min-h-screen bg-black text-white p-6"><div className="max-w-4xl mx-auto space-y-6">
    <h1 className="text-4xl font-extrabold text-[#D4AF37]">BWE 0.5% Challenge</h1>
    ...
    <section className="rounded border border-white/20 p-4">
      {stats?.ok ? <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm"><div><b>{stats.membersJoined}</b><div>Members</div></div><div><b>{stats.citiesRepresented}</b><div>Cities</div></div><div><b>{stats.businessOwnerInterestCount}</b><div>Business-owner interest</div></div><div><b>{stats.topCities?.[0]?.city || "-"}</b><div>Top city</div></div></div> : <p>Be one of the first members to join the BWE 0.5% Challenge.</p>}
    </section>
