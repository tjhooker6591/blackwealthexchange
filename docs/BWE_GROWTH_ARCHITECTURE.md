...
1. **User referral loop**
   ...
   - Output: referred signup -> first action -> first purchase

2. **Creator distribution loop**
   - Trigger: creator campaign links/collections
   - Mechanic: creator landing pages, tracked codes, affiliate payout
   - Output: creator-attributed signups + GMV

3. **Business acquisition-node loop**
   - Trigger: listed business shares BWE profile/QR
   - Mechanic: profile trust score, reviews, share assets
   - Output: profile traffic -> leads/orders -> more sharing

4. **Seller commerce loop**
   - Trigger: seller onboarding + first product
   - Mechanic: storefront, recommendations, wishlist, post-purchase share
   - Output: first order -> repeat buyer -> seller retention

5. **Employer-talent loop**
   - Trigger: employer post/intake + talent profile
   - Mechanic: hiring funnel + consulting service + spotlights
...
- Employers: postings/intake -> candidates -> hires -> repeat posts
- Talent: profile -> applications -> visibility -> outcomes
