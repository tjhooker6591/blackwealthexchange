...
export default function FoundingMembershipPage() {
  ...
  const confirmedBusiness = useMemo(
    () =>
      (data?.businesses || []).find(
        (business) => business.id === confirmedBusinessId,
      ) || null,
  ...
  const beginCheckout = async () => {
    ...
    try {
      ...
      setCheckoutState("checkout_error");
      setCheckoutMessage(
        e?.message || "Unable to start checkout. Please retry.",
      );
      setSubmitting(false);
    }
  };
          ...
          <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              ...
              <p className="mt-2 text-sm text-white/70">
                ...
                BWE business. Availability is provided by the server for
                guidance and rechecked again during checkout.
              </p>

              {loading ? (
                <p className="mt-4 text-white/60">Loading businesses…</p>
              ) : null}
              {error ? (
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!business.claimable) {
                          setError(
                            `${business.businessName} is not currently available for a new founding membership claim.`,
                          );
                          return;
                        }
                      ...
                      aria-pressed={active}
                      aria-describedby={
                        active ? `selected-business-${business.id}` : undefined
                      }
                    >
              ...
              <div className="mt-5 rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-4">
                ...
                {selectedBusiness ? (
                  <div className="mt-2 space-y-2 text-sm text-white/80">
                    ...
                    <div className="text-xs text-white/45">
                      Server availability:{" "}
                      {selectedBusiness.claimable ? "Claimable" : "Unavailable"}
                    </div>
                  </div>
                ) : (
                  <div className="mt-2 text-sm text-white/65">
                    Choose one public business to continue.
                  </div>
                )}
              ...
              <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
                ...
                {confirmedBusiness ? (
                  ...
                  <div className="mt-3 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/65">
                    Select and confirm one business above before checkout is
                    enabled.
                  </div>
                )}
